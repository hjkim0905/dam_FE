...
import { EVENT, personPropertiesOf, roomGroupOf } from '@/lib/analytics';
...
import { identify, forget, track } from '@/lib/track';
...
export default function SessionProvider({ children }: { children: ReactNode }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setProfile(found);
      identify(String(found.id), personPropertiesOf(found), roomGroupOf(found));
      ready();
    } catch (error) {
      // 토큰이 죽은 것과 연결이 끊긴 것은 다르다. 끊긴 것으로 세션을 버리면
      // 지하철에서 앱을 연 사람이 애플 로그인부터 다시 해야 한다.
      if (isSessionGone(error)) {
        track(EVENT.sessionExpired);
        leave();
        return;
      }
      // 버전이 낡은 것은 연결 문제가 아니다. 다시 시도해 봐야 같은 답이 온다.
      if (needsUpdate(error)) {
        track(EVENT.updateRequired);
        setOutdated(true);
        ready();
        return;
      }
      track(EVENT.serverUnreachable);
      setUnreachable(true);
  ...
  const goToStore = () => {
    track(EVENT.storeOpened);
    openStore();
  };

  if (outdated) return <UpdateRequired onUpdate={goToStore} />;
